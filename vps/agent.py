...
last_reported_bytes = {}
argo_tunnels = {}
prev_cpu_total = 0; prev_cpu_idle = 0; prev_rx = 0; prev_tx = 0; prev_net_time = 0
...
def get_system_status():
    ...
    stats = {"cpu": 0, "mem": 0, "disk": 0, "uptime": "Unknown", "load": "0.00 0.00 0.00", "net_in_speed": 0, "net_out_speed": 0, "tcp_conn": 0, "udp_conn": 0}
            ...
            for line in f:
                if line.startswith('cpu '):
                    ...
                    if prev_cpu_total > 0:
                        ...
                        if diff_total > 0: stats["cpu"] = int(100.0 * (1.0 - diff_idle / diff_total))
                    prev_cpu_total = total; prev_cpu_idle = idle
                    break
    except Exception: pass

    try:
        with open('/proc/meminfo', 'r') as f: mem_data = f.read()
        ...
        if total_match and avail_match:
            stats["mem"] = round(((float(total_match.group(1)) - float(avail_match.group(1))) / float(total_match.group(1))) * 100, 2)
    except Exception: pass

    try:
        st = os.statvfs('/')
        if st.f_blocks > 0: stats["disk"] = int(((st.f_blocks - st.f_bfree) * st.f_frsize / (st.f_blocks * st.f_frsize)) * 100)
    except Exception: pass

    try:
        with open('/proc/uptime', 'r') as f:
            up_s = float(f.readline().split()[0])
            stats["uptime"] = f"{int(up_s // 86400)} Day, {int((up_s % 86400) // 3600)}:{int((up_s % 3600) // 60):02d}"
    except Exception: pass

    try:
        with open('/proc/loadavg', 'r') as f: stats["load"] = " ".join(f.readline().split()[:3])
    except Exception: pass
    ...
    try:
        ...
        with open('/proc/net/dev', 'r') as f:
            ...
            for line in lines:
                ...
                if len(parts) > 10 and not parts[0].startswith('lo'):
                    rx_now += int(parts[1]); tx_now += int(parts[9])
        now_t = time.time()
        ...
        prev_rx = rx_now; prev_tx = tx_now; prev_net_time = now_t
    except Exception: pass
    return stats
...
def get_port_traffic(port, protocol="tcp"):
    ...
    try:
        if port not in checked_ports:
            ...
            check_out = f"iptables -C OUTPUT -p {protocol} --sport {port} -j ACCEPT"
        ...
        out_out = subprocess.check_output(f"iptables -nvx -L OUTPUT | grep 'spt:{port}' | grep 'ACCEPT'", shell=True).decode()
...
def report_status(current_nodes, argo_urls):
    ...
    node_traffic_deltas = []; current_ids = set()

    for node in current_nodes:
        nid = node["id"]; port = node["port"]; current_ids.add(nid)
    ...
    for old_id in list(last_reported_bytes.keys()):
        if old_id not in current_ids: del last_reported_bytes[old_id]
